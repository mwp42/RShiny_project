...
function(input, output, session) {
  ...
  output$map <- renderGvis({
    map<-gvisGeoChart(commodity_T5_maps, locationvar='country', colorvar='balance',
  ...
  observe({
    category <- unique(commodity_T5 %>%
                     filter(commodity_T5$year.trade == input$year.trade) %>%
                     .$category)
  ...
  output$bar_graph <- renderPlot({
    commodity_T5 %>%
      filter(.,commodity_T5$year.trade == input$year.trade & commodity_T5$category == input$category)%>%
  ...
  output$statistics <- renderPlot({
    commodity_T5 %>%
      filter(.,commodity_T5$country == input$country, year.trade==1996|year.trade==1998|year.trade==2000|year.trade==2002|year.trade==2004|year.trade==2006|year.trade==2008|year.trade==2010|year.trade==2012|year.trade==2014|year.trade==2016)%>%
      group_by(., year.trade)%>%
  ...
  observe({
    country <- unique(commodity_T5 %>%
                         filter(commodity_T5$year.trade == input$year.trade1) %>%
                         .$country1)
    updateSelectizeInput(
      session, "country",
      choices = country,
